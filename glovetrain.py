...
import scipy
# add txt to pre-process
from torch import nn


def getCorpus(filetype, size, MAX_VOCAB_SIZE):
    if filetype == 'dev':
        filepath = './test8.dev.txt'
    elif filetype == 'test':
        filepath = './test8.test.txt'
    else:
        filepath = './test8.train.txt'
...
def buildCooccuranceMatrix(text, word_to_idx, WINDOW_SIZE):
    ...
    for i, center_word_id in enumerate(text_ids):
        ...
        win_idx = window_indices
        window_indices = [i % maxlength for i in window_indices]
        print("i=%d,id=%s,win_idx=%s,win_idx=%s" % (i, center_word_id,win_idx,window_indices))
